...
function updateEmployeeRole(employee,role) {
    return new Promise(function (resolve, reject) {
        ...
        db.query(`UPDATE employee SET role_id = ${roleId} WHERE CONCAT(first_name, " ", last_name )= "${employee}";`, (err, result) => {
            if (err) {
                reject(err);
            }
             console.log("Success!");
...
// change from 72 to reject from consloe.log
...
function addDepartment(name) {
    return new Promise(function (resolve, reject) {
        db.query(`INSERT INTO department (name) VALUE ("${name}");`, (err, result) => {
            ...
            resolve(`Added ${name} to department`);
        })
    })
}
